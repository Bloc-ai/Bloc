...
### Inspecting Persistent Engine Logs

If a model fails to start or crashes during execution, the CLI automatically saves structured, persistent logs for the engine process. This makes it easy to diagnose obscure errors or out-of-memory crashes that occur inside the engine container. 

You can find all engine logs saved on your local machine at:
```bash
~/.cache/bloc/logs/
```
