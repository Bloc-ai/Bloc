...
The Bloc CLI is a unified terminal client used to search, run, and manage local AI environment runs.
...
```bash
bloc run arnav/qwen3-30b-moe-8gb-cpu-offload
```
...
* **`bloc runtime list`**: List all locally installed python virtual environments created by `native` vLLM runs with Python version, installation date, and size.
